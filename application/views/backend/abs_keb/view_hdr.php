...
function absKeb(id)
{
	url = "<?php echo site_url().'backend/abs_keb/view/'?>"+id;
...
function autocomplet() {
	...
	if (keyword.length >= min_length) {
		$.ajax({
			url: '<?php echo site_url().'backend/abs_keb/searchJemaat'?>',
			type: 'POST',
							...
							<table class="table table-hover">
								...
								<tr>
									...
									<div>
										...
										<?php echo anchor('backend/abs_keb/del_abs/'.$row["hdr_keb_keb_id"].'/'.$row["hdr_keb_id"],'Hapus',array('onclick' => "return confirm('Anda yakin menghapus data ini?')",'class' => 'btn btn-danger btn-xs'));?>
									</div>
									</td>
...
<div class="modal fade" id="myModalAbsen" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
			...
			<form action="<?php echo site_url().'backend/abs_keb/add_abs';?>" role="form" method="post" enctype="multipart/form-data">
